...
export class NFTURI {
    constructor(s) {
        Assert.check(MitumConfig.MAX_URI_LENGTH.satisfy(s.length), MitumError.detail(ECODE.INVALID_PARAMETER, "NFT-URI's length is out of range."));
        this.s = s;
    }
    toBuffer() {
...
export class CollectionName {
    constructor(s) {
        Assert.check(typeof s === "string", MitumError.detail(ECODE.INVALID_PARAMETER, "Collection-name's type must be of type 'string'."));
        Assert.check(MitumConfig.COLLECTION_NAME_LENGTH.satisfy(s.length), MitumError.detail(ECODE.INVALID_PARAMETER, "Collection-name's length is out of range."));
        this.s = s;
    }
    equal(name) {
        if (!name) {
