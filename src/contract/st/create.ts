...
const CreateSecurityTokensFactHint =
...
export class CreateSecurityTokensFact extends OperationFact<CreateSecurityTokensItem> {
  constructor(
    token: string,
    sender: string,
    ...
    items.forEach((item) => {
      Assert.check(
        item instanceof CreateSecurityTokensItem,
        MitumError.detail(
          ECODE.INVALID_PARAMETER,
    ...
    Assert.check(
      items.length <= MaxCreateSecurityTokensItems,
      MitumError.detail(
        ECODE.INVALID_PARAMETER,
