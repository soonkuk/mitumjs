...
export class Block {
  ...
  async getAllBlocks(): Promise<AxiosResponse | null> {
    return await blockInfo.getAllBlocksInfo(this._node);
  }

  // get block information by block number or hash
  async getBlock(block: number | string): Promise<AxiosResponse | null> {
  ...
  async getOperation(block: number): Promise<AxiosResponse | null> {
